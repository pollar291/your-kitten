...
CREATE_POSTER_IMAGE = "INSERT INTO poster_images (poster_id, image, mime_type) VALUES (%s, %s, %s) RETURNING id;"
...
GET_POSTERS = "SELECT p.id, p.title, p.address, p.phone, p.status, p.description, (SELECT jsonb_agg(pi.id) FROM poster_images pi WHERE p.id = pi.poster_id) AS image_ids FROM posters p LIMIT %s OFFSET %s"

GET_POSTER_IMAGE = "SELECT pi.image, pi.mime_type FROM poster_images pi WHERE pi.id = %s"